...
import { DiabetesType, HealthRecordDto, GlucoseReading } from "@/features/patient/types";
...
import { usePatientResumen } from "./use-patient-resumen";
...
export function useChartPatientData(): ChartPatientData {
  const { patientId } = useAuthStore();

  const { data: profile, isLoading: loadingProfile } = usePatientProfile(patientId ?? '');
  const { data: resumen, isLoading: loadingResumen } = usePatientResumen(patientId ?? '');
  ...
  return {
    records,
    diabetesType: mapDiabetesType(resumen?.perfil.tipoDiabetes ?? 'none'),
    gender: profile?.gender ?? null,
    isLoading: loadingProfile || loadingResumen || loadingDaily || loadingLab,
  };
}
