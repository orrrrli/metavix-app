...
import { imcLevel, type ClinicalLevel } from "@/shared/utils/status-colors";
...
export function ResultadoIMC({ imc, categoria }: ResultadoIMCProps) {
  ...
  const level: ClinicalLevel = imcLevel(categoria);
  const colorClass: Record<ClinicalLevel, string> = {
  ...
  // El "danger" usa rojo saturado por gravedad clínica, con tinte más oscuro en dark.
  const textClass = categoria.includes("grado III")
  ...
  return (
    <div className="bg-card border rounded-xl p-8 shadow-sm flex flex-col items-center justify-center text-center space-y-6 animate-in zoom-in-95 duration-500 h-full">
      <div className="space-y-1">
        ...
        <p className={cn("text-xl font-medium pt-2", textClass)}>
          {categoria}
        </p>
      </div>
      ...
      <div className="w-full max-w-sm pt-4">
        <div className="relative h-4 bg-muted rounded-full overflow-hidden flex">
          <div className="w-[11.6%] bg-amber-300 dark:bg-amber-700" title="Bajo peso (<18.5)"></div>
          <div className="w-[21.3%] bg-emerald-400 dark:bg-emerald-700" title="Normal (18.5-24.9)"></div>
          <div className="w-[16.6%] bg-orange-400 dark:bg-orange-700" title="Sobrepeso (25-29.9)"></div>
          <div className="w-[16.6%] bg-red-400 dark:bg-red-700" title="Obesidad I (30-34.9)"></div>
          <div className="w-[16.6%] bg-red-500 dark:bg-red-800" title="Obesidad II (35-39.9)"></div>
          <div className="flex-1 bg-red-700 dark:bg-red-900" title="Obesidad III (>=40)"></div>
        </div>
        ...
        <div className="relative h-6 mt-1 mx-2">
          <div
            ...
            style={{ left: `${percent}%` }}
          >
            <div className="w-0 h-0 border-l-[6px] border-r-[6px] border-b-[8px] border-transparent border-b-foreground dark:border-b-foreground"></div>
