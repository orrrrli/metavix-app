...
export default function NewRecordPage() {
  ...
  const { patients, addRecord, updatePatient } = useMockDb();
  ...
  const onSubmit = async (data: RecordFormValues) => {
    if (!userId || !patientProfile) return;
    setIsSubmitting(true);
    
    // Si la paciente es mujer, actualizamos el estado de embarazo a nivel Perfil global
    if (patientProfile.gender === 'F' && data.embarazada !== undefined) {
      updatePatient(userId, { pregnancyStatus: data.embarazada });
    }
